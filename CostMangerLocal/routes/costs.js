...
// rout for get all costs of current user
...
router.get('/:userId/sort', async function (req, res) {
    ...
    try {
        ...
        if (req.query.sortBy === 'category') {
            costs = await Cost.find({userId: req.params.userId})
            .sort({category: 'asc'});
        }
...
// rout for handling  cost creation request
router.post('/:userId/', async function (req, res) {
    ...
    try {
        ...
        newCost.save().then(newCost => {
            console.log('new cost was created: ' + newCost);
...
router.get('/:userId/:costId', async function (req, res) {

    try {
        const targetCostToDisplay = await Cost.findById(req.params.costId);

        // if the targetCostToDisplay is empty 
        //-> it means thar cost with given id is not exist
...
router.get("/:userId/:costId/edit", async function (req, res) {
    ...
    try {
        ...
        res.render("costs/editCost", {
            ...
            cost: targetCostToUpdated,
            categories: categories, 
            backLink: `/costs/${req.params.userId}/${req.params.costId}`
        });
    } catch (error) {
        res.send(error);
    }
...
router.put('/:userId/:costId', async function (req, res) {
    try {
        ...
        const targetCostToUpdated = await Cost
        .findByIdAndUpdate(req.params.costId, updatedCost);

        if (targetCostToUpdated == null) {
            res.send("error did not find cost with given id");
        } else {
            res.redirect(`/costs/${req.params.userId.toString()}
            /${req.params.costId.toString()}`);
        }
    } catch (err) {
        res.send("error: " + err);
    }
...
router.delete('/:userId/:costId', async function (req, res) {
    ...
    try {
        ...
        //-> the return value is null if there is no cost with given id
        if (costToDelete !== null) {
            console.log("Cost to delete: " + costToDelete);
            ...
            Cost.find({userId: req.params.userId}, function (err, fittingCosts) {
                if (err) {
                    ...
                    User.findByIdAndUpdate(req.params.userId,
                         {"costs": fittingCosts}, function (err, result) {
                        if (err) {
                            res.send(err);
                        } else {
